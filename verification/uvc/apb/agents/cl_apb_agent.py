...
from .cl_apb_consumer_driver import cl_apb_consumer_driver
    ...
    def build_phase(self):
        ...
        if self.cfg.is_active == uvm_active_passive_enum.UVM_ACTIVE:
            ...
            if self.cfg.driver == DriverType.PRODUCER:
                ...
                self.logger.critical(
                    f"Driver type not set in cfg {self.cfg.get_full_name()}"
                )
        ...
        if self.cfg.seq_item_override == SequenceItemOverride.DEFAULT:
            uvm_factory().set_type_override_by_type(
                cl_apb_seq_item,
                apb_change_width(self.cfg.ADDR_WIDTH, self.cfg.DATA_WIDTH),
            )
