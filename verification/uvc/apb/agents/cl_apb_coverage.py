...
class cl_apb_coverage(uvm_subscriber):
    ...
    def build_phase(self):
        ...
        self.cg_trans_kind = covergroup_trans_kind(
            f"{self.get_full_name()}.cg_trans_kind", self.cfg
        )
        self.cg_trans_data = covergroup_trans_data(
            f"{self.get_full_name()}.cg_trans_data", self.cfg
        )
        self.cg_trans_delay = covergroup_trans_delays(
            f"{self.get_full_name()}.cg_trans_delay"
        )
...
class covergroup_trans_kind(object):
    def __init__(self, name, cfg):
        ...
        self.with_sample(
            op=vsc.bit_t(1), addr=vsc.bit_t(self.cfg.ADDR_WIDTH), slverr=vsc.bit_t(1)
        )

        self.trans_op = vsc.coverpoint(
            self.op, bins={"RD": vsc.bin(0), "WR": vsc.bin(1)}
        )

        self.trans_slverr = vsc.coverpoint(
            self.slverr, bins={"NO_ERROR": vsc.bin(0), "ERROR": vsc.bin(1)}
        )
...
class covergroup_trans_data(object):
    def __init__(self, name, cfg):
        ...
        self.with_sample(op=vsc.bit_t(1), data=vsc.bit_t(1), bitno=vsc.bit_t(no_bits))

        self.trans_bitno = vsc.coverpoint(self.bitno, cp_t=vsc.bit_t(no_bits))

        self.trans_data = vsc.coverpoint(
            self.data, bins={"ZERO": vsc.bin(0), "ONE": vsc.bin(1)}
        )

        self.trans_data_type = vsc.coverpoint(
            self.op, bins={"RD": vsc.bin(0), "WR": vsc.bin(1)}
        )

        self.cross_data_bits_type = vsc.cross(
            [self.trans_bitno, self.trans_data_type, self.trans_data]
        )
...
class covergroup_trans_delays(object):
    def __init__(self, name):
        ...
        self.with_sample(wait_len=vsc.bit_t(4))
