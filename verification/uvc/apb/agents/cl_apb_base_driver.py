...
class cl_apb_base_driver(uvm_driver):
    ...
    async def get_and_drive_transaction(self):
        # Start driver loop
        if (
            self.cfg.driver is not DriverType.PRODUCER
            and self.cfg.driver is not DriverType.CONSUMER
        ):
            raise UVMFatalError(
    ...
    async def driver_loop(self):
        while True:
            ...
            self.logger.info(f"[RQ] {self.req!s}")
            self.logger.debug(f"[<=] {self.req!r}")
            ...
            self.logger.info(f"[RP] {self.rsp!s}")
            self.logger.debug(f"[=>] {self.rsp!r}")
